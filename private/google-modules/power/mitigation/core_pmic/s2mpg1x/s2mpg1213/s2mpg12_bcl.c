...
{
	if (is_meter)
		return s2mpg12_write_reg(bcl_dev->main_meter_i2c, reg, value);
	return s2mpg12_write_reg(bcl_dev->main_pmic_i2c, (u8)reg, value);
}

int core_pmic_main_read_register(struct bcl_device *bcl_dev, u16 reg, u8 *value, bool is_meter)
{
	if (is_meter)
		return s2mpg12_read_reg(bcl_dev->main_meter_i2c, reg, value);
	return s2mpg12_read_reg(bcl_dev->main_pmic_i2c, (u8)reg, value);
}
...
u16 core_pmic_main_store_uvlo(struct bcl_device *bcl_dev, unsigned int val, size_t size)
{
	...
	if (pmic_read(CORE_PMIC_MAIN, bcl_dev, SMPL_WARN_CTRL, &value)) {
		dev_err(bcl_dev->device, "S2MPG1213 read %#x failed.", SMPL_WARN_CTRL);
		return -EBUSY;
	}

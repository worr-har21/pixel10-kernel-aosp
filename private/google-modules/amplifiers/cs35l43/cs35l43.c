...
#include "wm_adsp.h"
#include "cs35l43.h"
#include "include/sound/cs35l43.h"
...
static const char * const cs35l43_noise_gate_delay_text[] =  {
	"5ms", "10ms", "25ms", "50ms", "100ms", "250ms", "500ms", "1000ms"};

static SOC_ENUM_SINGLE_DECL(cs35l43_noise_gate_delay,
			    CS35L43_NG_CONFIG, CS35L43_NG_DELAY_SHIFT,
			    cs35l43_noise_gate_delay_text);
...
static int cs35l43_bp_current_limit_get(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component;
	struct cs35l43_private *cs35l43;
	...
	ret = regmap_read(cs35l43->regmap, soc_enum->reg, (int *)&value);
	ucontrol->value.integer.value[0] =
		(long)((value >> CS35L43_BST_IPK_SHIFT) & CS35L43_BST_IPK_MASK);

	return ret;
}

static int cs35l43_bp_current_limit_put(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component;
	struct cs35l43_private *cs35l43;
	...
	ret = regmap_update_bits(cs35l43->regmap, soc_enum->reg,
			CS35L43_BST_IPK_MASK,
			((int)ucontrol->value.integer.value[0] << CS35L43_BST_IPK_SHIFT));

	if (ret) {
		dev_err(cs35l43->dev, "Failed to write peak current limit\n");
		return ret;
	} else
		dev_info(cs35l43->dev, "%s: value %d\n",
				__func__, cs35l43->pdata.bst_ipk);

	return 0;
}

static const char * const cs35l43_boost_current_limit_text[] = {
	...
	"4.40A", "4.45A", "4.50A"};
static SOC_ENUM_SINGLE_DECL(current_limit, CS35L43_BST_IPK_CTL,
				CS35L43_BST_IPK_SHIFT,
				cs35l43_boost_current_limit_text);
...
static const struct snd_kcontrol_new cs35l43_aud_controls[] = {
	...
	SOC_ENUM_EXT("Boost Peak Current Limit", current_limit,
		     cs35l43_bp_current_limit_get, cs35l43_bp_current_limit_put),
	SOC_ENUM("Noise Gate Delay", cs35l43_noise_gate_delay),
};
...
static int cs35l43_log_dsp_err(struct cs35l43_private *cs35l43)
{
	...
	dev_info(cs35l43->dev, "%s: Dump register ===\n", __func__);
...
#if 0 /* TODO(b/355326394): revert this change after the bug is fixed. */
	cs35l43_check_mailbox(cs35l43);
#endif
...
{
	unsigned int pdn_done;

	if (cs35l43->hibernate_state != CS35L43_HIBERNATE_AWAKE)
		return 0;

	dev_info(cs35l43->dev, "%s: hibernate_state = %d\n", __func__,
			cs35l43->hibernate_state);
	...
	regmap_read(cs35l43->regmap, CS35L43_IRQ1_EINT_1, &pdn_done);
	if (!(pdn_done & CS35L43_MSM_PDN_DONE_EINT1_MASK))
		dev_err(cs35l43->dev, "Power-down incomplete\n");
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (cs35l43->low_pwr_mode == CS35L43_LOW_PWR_MODE_STANDBY) {
			regmap_write(cs35l43->regmap, CS35L43_IRQ1_EINT_1,
					CS35L43_MSM_PDN_DONE_EINT1_MASK);
			usleep_range(1000, 1100);
			regmap_write(cs35l43->regmap, CS35L43_GLOBAL_ENABLES, 0);
			usleep_range(1000, 1100);
			regmap_read(cs35l43->regmap, CS35L43_IRQ1_EINT_1, &pdn_done);
			if (!(pdn_done & CS35L43_MSM_PDN_DONE_EINT1_MASK)) {
				regmap_read(cs35l43->regmap, CS35L43_AMP_CTRL, &temp_vol);
				regmap_write(cs35l43->regmap, CS35L43_AMP_CTRL, 0x00008000);
				regmap_write(cs35l43->regmap, CS35L43_AMP_CTRL, temp_vol);
				regmap_read(cs35l43->regmap, CS35L43_IRQ1_EINT_1, &pdn_done);
				if (!(pdn_done & CS35L43_MSM_PDN_DONE_EINT1_MASK)) {
					dev_err(cs35l43->dev, "Power-down incomplete\n");
					ret = -EINVAL;
...
static const struct snd_soc_dapm_widget cs35l43_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SWITCH("Main AMP Enable", SND_SOC_NOPM, 0, 1, &amp_enable_ctrl),
};
...
static const struct snd_soc_dapm_route cs35l43_audio_map[] = {
	...
	{"Main AMP Enable", "Switch", "AMP Playback"},

	{"ASPRX1", NULL, "Main AMP Enable"},
	{"ASPRX2", NULL, "Main AMP Enable"},
	{"VMON ADC", NULL, "Main AMP Enable"},
	{"IMON ADC", NULL, "Main AMP Enable"},
	{"VPMON ADC", NULL, "Main AMP Enable"},
	{"VBSTMON ADC", NULL, "Main AMP Enable"},
	{"TEMPMON ADC", NULL, "Main AMP Enable"},
};
...
static int cs35l43_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	dev_dbg(cs35l43->dev, "%s: format = %x\n", __func__, fmt);
...
static int cs35l43_component_write(struct snd_soc_component *component,
...
static unsigned int cs35l43_component_read(struct snd_soc_component *component,
				unsigned int reg)
{
	struct cs35l43_private *cs35l43 =
				snd_soc_component_get_drvdata(component);
				  ...
				  struct cs35l43_private *cs35l43)
{
	...
	dev_info(dev, "low_pwr_mode = %d\n", cs35l43->low_pwr_mode);

	return 0;
}

static const char * const dapm_names[] = { "SPK", "VP", "AMP Playback", "AMP Capture" };

static void cs35l43_component_ignore_suspend(struct snd_soc_component *component)
{
	struct snd_soc_dapm_context *dapm = snd_soc_component_get_dapm(component);
	int i;

	for (i = 0; i < ARRAY_SIZE(dapm_names); i++) {
		pr_info("snd_soc_dapm_ignore_suspend dapm name[%s]", dapm_names[i]);
...
static int cs35l43_component_probe(struct snd_soc_component *component)
{
	...
	cs35l43_component_ignore_suspend(component);
	return ret;
}
...
static int cs35l43_pcm_startup_nop(struct snd_pcm_substream *substream,
			       struct snd_soc_dai *dai)
{
...
static const struct snd_soc_dai_ops cs35l43_ops_nop = {
	.startup = cs35l43_pcm_startup_nop,
};

static struct snd_soc_dai_driver cs35l43_dai_nop[] = {
	{
		.name = "cs35l43-pcm",
		.id = 0,
		.playback = {
...
static const struct snd_kcontrol_new cs35l43_aud_controls_nop[] = {
	SOC_SINGLE("AMP Disabled", SND_SOC_NOPM, 0, 0, 0),
};

static const struct snd_soc_component_driver soc_component_dev_cs35l43_nop = {
	.controls = cs35l43_aud_controls_nop,
	.num_controls = ARRAY_SIZE(cs35l43_aud_controls_nop),
};
...
err:
	...
	if (snd_soc_register_component(cs35l43->dev,
			&soc_component_dev_cs35l43_nop,
			cs35l43_dai_nop, ARRAY_SIZE(cs35l43_dai_nop)) < 0) {
		dev_err(cs35l43->dev, "%s: Register codec failed\n", __func__);
	} else {
		dev_info(cs35l43->dev, "%s: Register nop codec\n", __func__);
		ret = 0;
	}

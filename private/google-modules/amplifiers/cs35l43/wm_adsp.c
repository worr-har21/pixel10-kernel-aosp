					 ...
					 const char *filetype)
{
	...
	if (dsp->tuning_has_prefix && asoc_component_prefix)
		*filename = kasprintf(GFP_KERNEL, "%s-%s-%s-%s.%s",
					  asoc_component_prefix, dsp->part, dsp->fwf_name,
					  wm_adsp_fw[dsp->fw].file, filetype);
	else if (dsp->tuning_has_prefix)
		*filename = kasprintf(GFP_KERNEL, "%s-%s-%s.%s",
					  dsp->part, dsp->fwf_name, wm_adsp_fw[dsp->fw].file,
					  filetype);

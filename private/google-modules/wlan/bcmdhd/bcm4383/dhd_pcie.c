...
dhd_bus_devreset(dhd_pub_t *dhdp, uint8 flag)
{
	...
	int bcmerror = 0;
	unsigned long flags;
	bool bus_stop_host_dev = FALSE;

	BCM_REFERENCE(bus_stop_host_dev);
	...
	if (flag == TRUE) { /* Turn off WLAN */
		...
		/* Do RC off first to avoid GEM timeout in phones */
		if (bus->is_linkdown) {
			bcmerror = dhdpcie_bus_stop_host_dev(bus);
			if (bcmerror) {
			...
			bus_stop_host_dev = TRUE;
		}
#endif /* DHD_TREAT_D3ACKTO_AS_LINKDWN || DHD_TREAT_D2H_CTO_AS_LINKDOWN */
...
#ifdef DHD_TREAT_D2H_CTO_AS_LINKDOWN
		if (bus->dhd->dongle_trap_data & D2H_DEV_TRAP_CTO) {
			DHD_ERROR(("%s treating dongle_trap_data "
				"D2H_CTO(%d) as PCIe link down \n",
				__FUNCTION__, bus->d2h_cto_as_linkdwn_cnt));
			dhd_plat_pcie_skip_config_set(TRUE);
			dhd_bus_set_linkdown(bus->dhd, TRUE);
			bus->d2h_cto_as_linkdwn_cnt++;
			/* wake up IOCTL wait event */
			dhd_wakeup_ioctl_event(bus->dhd, IOCTL_RETURN_ON_TRAP);
			bus->dhd->hang_reason = HANG_REASON_PCIE_LINK_DOWN_RC_DETECT;
			dhd_os_send_hang_message(bus->dhd);
...
#endif /* DHD_TREAT_D2H_CTO_AS_LINKDOWN */

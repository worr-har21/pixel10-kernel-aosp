...
// mfd core driver for DA9188 & DA9189 PMICs.
#include <linux/err.h>
...
/**
 * struct da9188_mfd_context - A single connection to PMIC registers via CPM.
 * @dev: The device.
 */
struct da9188_mfd_context {
	struct device *dev;
};

static const struct mfd_cell da9188_mfd_devices[] = {
	{
		.name = "da9188-regulator",
		.of_compatible = "google,da9188-regulator",
	},
	{
		.name = "da9188-rtc",
		.of_compatible = "google,da9188-rtc",
	},
	{
		.name = "da9188-gpio",
		.of_compatible = "google,da9188-gpio",
	},
	{
		.name = "da9188-vgpio",
		.of_compatible = "google,da9188-vgpio",
	},
};
...
int da9188_mfd_mbox_request(struct device *dev, struct pmic_mfd_mbox *mbox)
{
	int ret;
...
EXPORT_SYMBOL_GPL(da9188_mfd_mbox_request);
...
EXPORT_SYMBOL_GPL(da9188_mfd_mbox_release);
					   ...
					   struct mailbox_data *resp_data)
{
	...
	for (attempts = 1; attempts <= MAILBOX_MAX_ATTEMPTS;
		...
		dev_dbg(dev,
			...
			status, resp_payload->data_1, resp_payload->data_2);

		/* Validate the response status from CPM. */
		if (unlikely(status != MB_RSP_STS_OK)) {
			dev_err(dev, "Mailbox response status failure: %u, msg to CPM was cmd: %u addr: %u data[0]: %#x [1]: %#x\n",
				status, cmd, id_or_addr, req_msg.payload[0], req_msg.payload[1]);
			ret = -EPIPE;
			continue;
		}
...
EXPORT_SYMBOL_GPL(da9188_mfd_mbox_send_req_blocking_read);
...
int da9188_mfd_mbox_send_req_blocking(struct device *dev,
				      ...
				      struct mailbox_data req_data)
{
	return da9188_mfd_mbox_send_req_blocking_read(dev, mbox,
						      mbox_dst, target, cmd,
						      id_or_addr, req_data,
						      NULL);
}
EXPORT_SYMBOL_GPL(da9188_mfd_mbox_send_req_blocking);

static int da9188_mfd_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct da9188_mfd_context *da9188_mfd;
	int ret;
	...
	da9188_mfd = devm_kzalloc(dev, sizeof(struct da9188_mfd_context),
				  GFP_KERNEL);
	if (!da9188_mfd)
		return -ENOMEM;

	platform_set_drvdata(pdev, da9188_mfd);
	da9188_mfd->dev = dev;

	ret = devm_mfd_add_devices(dev, PLATFORM_DEVID_NONE, da9188_mfd_devices,
				   ARRAY_SIZE(da9188_mfd_devices), NULL, 0,
				   NULL);
	if (ret)
		return ret;

	dev_dbg(dev, "DA9188 MFD driver probe done\n");

	return 0;
}

static const struct of_device_id da9188_mfd_of_match[] = {
	{ .compatible = "google,da9188mfd" },
	{ /* sentinel */ },
};
MODULE_DEVICE_TABLE(of, da9188_mfd_of_match);

static struct platform_driver da9188_mfd_driver = {
	.probe = da9188_mfd_probe,
	.driver = {
		.name = "da9188-mfd",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(da9188_mfd_of_match),
	},
};
module_platform_driver(da9188_mfd_driver);

MODULE_AUTHOR("Google LLC");
MODULE_DESCRIPTION("DA9188 PMIC multi-function core driver");
MODULE_LICENSE("GPL");

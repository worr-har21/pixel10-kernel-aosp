...
"""Script to parse the IO sheet and generate the linux_drivers/pinctrl/google/pinctrl-rdo.c file.

The sheet needs to be downloaded from go/<target>-io and should be saved in the same
directory as that of the parser.

Current sheet date: Sep 20, 2022.
...
class Pin:
  ...
  def __init__(self, pad_nr, pad_name, pad_type, pin_functions):
...
class Sswrp:
  ...
  def __init__(self, name):
    self.name = name.split("_")[0].lower()
    self.pins = []
    self.functions = {}
    self.sswrp_data_name = "google_rdo_pinctrl_" + self.name
    self.pins_array_name = "google_rdo_" + self.name
    self.pin_groups_name = "google_rdo_" + self.name + "_groups"

    self.pin_functions_name = "google_rdo_" + self.name + "_functions"
    self.functions_enum_name = "google_rdo_pinmux_" + self.name + "_functions"
  ...
  def print_pinctrl_pins(self):
    ...
    for i in range(0, len(self.pins)):
      print("\tPINCTRL_PIN_TYPE({},\"{}\", {}),".format(i,
                                                        self.pins[i].pad_name,
                                                        self.pins[i].pad_type))
    print("};\n")
...
class Common:
  ...
  def print_compatible(self):
    ...
    def print_compatible_single(sswrp):
      compatible_name = "google," + "rdo-" + sswrp.name + "-pinctrl"

      print("\t{{ .compatible = \"{}\",".format(compatible_name))
      print("\t  .data = &{} }},".format(sswrp.sswrp_data_name))

    match_table_name = "google_rdo_of_match"
...
class Parser:
  ...
  def __init__(self):
    ...
    self.valid_sswrps = [
        "HSION_BK", "HSIOS_BK", "LSIOS_BK", "LSIOE_BK", "LSION_BK", "CPM_BK",
        "AOC_BK", "GDMC_BK", "DPU_BK"
    ]
    # Atleast one pin of this SSWRP has been found
    ...
    self.valid_functions = {
        ...
        "hsios": [
            "gpio", "clkbuf", "ufs", "sd_data", "sd_cmd", "sd_fbclk", "sd_clk",
            "atb1", "pcie1", "vsync"
        ],
        "lsios": [
  ...
  def get_functions(self, row, pad_name):
    ...
    for i in range(0, max_nr_funcs):
      ...
      if self.ws[function_cell].value is not None:
  ...
  def parse_data(self):
    ...
    for row in range(1, self.row_count + 1):
      sswrp_cell = "D" + str(row)
      sswrp_name = self.ws[sswrp_cell].value
      pad_nr_cell = "B" + str(row)
      pad_name_cell = "C" + str(row)
      pad_name = self.ws[pad_name_cell].value
      libcellinfo_cell = "H" + str(row)
      libcellinfo = self.ws[libcellinfo_cell].value
      if "PRWDWUWSW" in str(libcellinfo):
          pad_type = "PIN_DRV_3_BITS_WITH_SLEW"
      elif "PDDWUWSW" in str(libcellinfo):
          pad_type = "PIN_DRV_4_BITS_NO_SLEW"
      else:
          pad_type = "PIN_TYPE_RESERVED"

      if self.is_pin_valid(pad_nr_cell, sswrp_name):
        self.check_and_append_sswrp(sswrp_name)
        self.sswrp_list[-1].pins.append(
            Pin(self.ws[pad_nr_cell].value, pad_name, pad_type,

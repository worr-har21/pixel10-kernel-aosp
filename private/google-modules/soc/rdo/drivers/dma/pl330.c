...
struct dma_pl330_chan {
	...
	spinlock_t lock;
	/* To protect against threaded power management */
...
static void pl330_tasklet(struct tasklet_struct *t)
{
	...
	list_for_each_entry_safe(desc, _dt, &pch->work_list, node)
		if (desc->status == DONE && desc->err == PL330_ERR_NONE) {
			if (!pch->cyclic)
...
static void pl330_issue_pending(struct dma_chan *chan)
{
	...
	/*
	 * Protect against multiple issue_pending calls ensuring
	 * pm_runtime_resume happens before the tasklet runs.
	 */
	mutex_lock(&pch->pm_lock);
	spin_lock_irqsave(&pch->lock, flags);
	if (list_empty(&pch->work_list)) {
		/*
	...
	if (power_up) {
		ret = pm_runtime_resume_and_get(pch->dmac->ddma.dev);
		if (ret) {
			spin_lock_irqsave(&pch->lock, flags);
			/*
			 * Since no error is returned by issue_pending hence add descriptors
			 * back to the submitted list and expect the client to timeout in case
			 * runtime_resume fails
			 */
			list_splice_tail_init(&pch->work_list, &pch->submitted_list);
			pch->active = false;
			spin_unlock_irqrestore(&pch->lock, flags);
			mutex_unlock(&pch->pm_lock);
			dev_err(pch->dmac->ddma.dev, "%s:%d pm_runtime_resume Failed (%d)!\n",
...
/*
 * Returns best possible burst size to be used to copy data from/to memory during a
 * peripheral transfer
 */
...
/*
 * Returns best possible burst length to be used to copy data from/to memory
 * during a peripheral transfer.
 * Returns 0 is burst length could not be computed
 */
static unsigned int get_periph_mem_brst_len(struct dma_pl330_desc *desc,
					    struct dma_pl330_chan *pch,
					    unsigned int mem_burst_size)
{
	/* Burst Lengths can be equal for equal burst sizes*/
	if (mem_burst_size != pch->burst_sz) {
		/*
		 * Attempt to adjust AxLen to keep number of bytes same in Load/Store
		 * if burst size towards memory is greater than that towards the peripheral.
		 * Find if there is a valid divident of burst length towards memory which
		 * keeps the amount of bytes moved same, if not 0 will be returned.
		 */
		if (mem_burst_size > pch->burst_sz)
			return pch->burst_len >> (mem_burst_size - pch->burst_sz);
		else
			return 0;
	}
	return pch->burst_len;
}
		...
		unsigned long flags)
{
	...
	struct dma_pl330_chan *pch = to_pchan(chan);
	unsigned int i, mem_burst_size, mem_burst_len;
	struct pl330_dmac *pl330 = pch->dmac;
	dma_addr_t dst;
	dma_addr_t src;
	...
	for (i = 0; i < len / period_len; i++) {
		...
		mem_burst_size = get_periph_mem_brst_sz(dma_addr, period_len, pch);
		mem_burst_len = get_periph_mem_brst_len(desc, pch, mem_burst_size);

		/*
		 * If computed burst length is invalid, Copy data in singles as per
		 * device requested burst size
		 */
		if (mem_burst_len < 1) {
			mem_burst_size = pch->burst_sz;
			mem_burst_len = pch->burst_len;
		}
		...
		case DMA_MEM_TO_DEV:
			...
			src = dma_addr;
			dst = pch->fifo_dma;
			desc->rqcfg.src_brst_size = mem_burst_size;
			desc->rqcfg.src_brst_len = mem_burst_len;
		...
		case DMA_DEV_TO_MEM:
			...
			desc->rqcfg.dst_brst_size = mem_burst_size;
			desc->rqcfg.dst_brst_len = mem_burst_len;
			break;
		default:
			break;
		}
		...
		unsigned long flg, void *context)
{
	...
	struct dma_pl330_chan *pch = to_pchan(chan);
	unsigned int mem_burst_size, mem_burst_len;
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		mem_burst_size = get_periph_mem_brst_sz(mem_addr, len, pch);
		mem_burst_len = get_periph_mem_brst_len(desc, pch, mem_burst_size);

		/*
		 * If computed burst length is invalid, Copy data is singles as per
...
static int pl330_pm_runtime_suspend(struct device *dev)
{
	...
	ret = google_update_ip_idle_status(pl330->ip_idle_index, STATE_IDLE);
	if (ret)
		dev_err(dev,
			"Couldn't set google_update_ip_idle_status, ret %d\n",
			ret);

	return 0;
}

static int pl330_pm_runtime_resume(struct device *dev)
{
	struct pl330_dmac *pl330 = dev_get_drvdata(dev);
	int ret;

	ret = google_update_ip_idle_status(pl330->ip_idle_index, STATE_BUSY);
	if (ret)
		dev_err(dev,
			"Couldn't set google_update_ip_idle_status, ret %d\n",
			ret);
...
pl330_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	/* get ip idle index */
	ret = of_property_read_u32(np, "ip-idle-index", &pl330->ip_idle_index);
	if (ret)
		dev_err(&adev->dev, "Couldn't get ip_idle_index\n");

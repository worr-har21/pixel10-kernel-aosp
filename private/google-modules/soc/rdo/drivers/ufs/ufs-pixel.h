...
#define PIXEL_UFS_MAGIC	0x5346556c65786950	/* PixelUFS */
...
struct pixel_ops {
	...
	int (*set_pwr_mode)(struct ufs_hba *hba,
			    const struct ufs_pa_layer_attr *pwr_mode);
	void (*get_pwr_mode)(struct ufs_hba *hba,
			     struct ufs_pa_layer_attr *pwr_mode);
	/* String containing PHY version. Caller responsible to free */
	char *(*get_phy_version)(struct ufs_hba *hba);
	/* String containing PHY release date. Caller responsible to free */
	char *(*get_phy_release_date)(struct ufs_hba *hba);
};
...
extern void pixel_ufs_notify_system_pm(struct ufs_hba *hba, bool is_suspend);
...
struct pixel_ufs {
	...
	/* maintain crypto slot usage */
	bool crypto_slot_in_use[KDN_SLOT_NUM];
};

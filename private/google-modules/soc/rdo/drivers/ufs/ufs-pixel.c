...
void pixel_ufs_notify_system_pm(struct ufs_hba *hba, bool is_suspend)
{
	pixel_update_power_event(hba, is_suspend ? PE_SYSTEM_SUSPEND : PE_SYSTEM_RESUME);
}
			...
			const char *buf, size_t count)
{
	...
	if (err || hrtimer_active(&ufs->manual_gc.hrtimer)) {
		...
		/* ufshcd_rpm_put_sync in delay_ms */
				...
				struct device_attribute *attr, char *buf)
{
	...
	phy_version = ufs->pixel_ops->get_phy_version(hba);
...
static DEVICE_ATTR_RO(phy_version);

static ssize_t phy_release_date_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	struct pixel_ufs *ufs = to_pixel_ufs(hba);
	...
	count = sysfs_emit(buf, "%s\n", phy_release_date ?: "N/A");

	kfree(phy_release_date);

	return count;
}
static DEVICE_ATTR_RO(phy_release_date);
...
static struct attribute *pixel_sysfs_pixel_attrs[] = {
	...
	&dev_attr_phy_release_date.attr,
	NULL,
};
...
void pixel_init_writebooster(struct ufs_hba *hba)
{
	/* Enable capability for writebooster */
...
#define PIXEL_POWER_MODE_ATTR(_name)				\
static ssize_t _name##_show(struct device *dev,			\
	struct device_attribute *attr, char *buf)		\
{								\
	struct ufs_hba *hba = dev_get_drvdata(dev);		\
	...
	struct ufs_pa_layer_attr pwr_mode;			\
								\
	if (!ufs->pixel_ops->set_pwr_mode ||			\
	    !ufs->pixel_ops->get_pwr_mode)			\
		return -EINVAL;					\
								\
	ufs->pixel_ops->get_pwr_mode(hba, &pwr_mode);		\
								\
	return sysfs_emit(buf, "%u\n", pwr_mode._name##_rx);	\
}								\
static ssize_t _name##_store(struct device *dev,		\
...
{								\
	struct ufs_hba *hba = dev_get_drvdata(dev);		\
	struct pixel_ufs *ufs = to_pixel_ufs(hba);		\
	struct ufs_pa_layer_attr new_pwr_mode;			\
	u32 value;						\
	int ret;						\
								\
	if (!ufs->pixel_ops->set_pwr_mode ||			\
	...
	if (kstrtouint(buf, 0, &value))				\
		return -EINVAL;					\
								\
	ufs->pixel_ops->get_pwr_mode(hba, &new_pwr_mode);	\
	new_pwr_mode._name##_rx = value;			\
	new_pwr_mode._name##_tx = value;			\
	ret = ufs->pixel_ops->set_pwr_mode(hba, &new_pwr_mode);	\
								\
	return ret ?: count;					\
}								\
static DEVICE_ATTR_RW(_name)					\

static ssize_t hs_rate_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	struct pixel_ufs *ufs = to_pixel_ufs(hba);
	...
	if (!ufs->pixel_ops->set_pwr_mode || !ufs->pixel_ops->get_pwr_mode)
		return -EINVAL;
...
static ssize_t hs_rate_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	struct pixel_ufs *ufs = to_pixel_ufs(hba);
...
static ssize_t change_pwr_mode_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
...
PIXEL_POWER_MODE_ATTR(gear);
PIXEL_POWER_MODE_ATTR(lane);
PIXEL_POWER_MODE_ATTR(pwr);
static DEVICE_ATTR_RW(hs_rate);
static DEVICE_ATTR_WO(change_pwr_mode);

static struct attribute *pixel_sysfs_power_mode[] = {
	&dev_attr_gear.attr,
	&dev_attr_lane.attr,
	&dev_attr_pwr.attr,
	&dev_attr_hs_rate.attr,
	&dev_attr_change_pwr_mode.attr,
	NULL,
};

static const struct attribute_group pixel_sysfs_power_mode_group = {
	.name = "power_mode",
	.attrs = pixel_sysfs_power_mode,
};
...
int pixel_init(struct ufs_hba *hba, struct device *pdev,
	       const struct pixel_ops *pixel_ops)
{
	struct pixel_ufs *ufs = to_pixel_ufs(hba);
	int ret;

	/* Add a magic number so we can find the ufs struct more easily. */
	ufs->magic_word = PIXEL_UFS_MAGIC;
	ufs->pixel_ops = pixel_ops;
	ufs->hba = hba;
	ufs->dev = pdev;

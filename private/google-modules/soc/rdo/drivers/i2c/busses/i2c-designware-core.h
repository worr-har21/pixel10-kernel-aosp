...
/* Set in xfer_init and reset when STOP detected */
#define STATUS_XFER_IN_PROGRESS			BIT(3)
#define STATUS_MASK				GENMASK(3, 0)
 ...
 * @ext_power_control: true if this bus requires external power control
 * @rinfo: I²C GPIO recovery information
 * @pinctrl: I2C instance pinctrl block
 * @cli_state: CLI pinctrl state
 * @operation_mode: Specifies interrupt (default) or hybrid xfer operation mode
 * @hybrid_threshold: In bytes. Specifies the threshold at which the transfer
 *	will use interrupt mode.  All values below this will use busy waiting.
 *
...
struct dw_i2c_dev {
	...
	enum i2c_dw_xfer_op_mode operation_mode;
	u32			hybrid_threshold;
};

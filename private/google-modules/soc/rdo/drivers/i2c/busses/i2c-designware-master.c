...
#define HYBRID_BUSY_WAIT_BYTES_THLD 10
/* Busy-wait for 300us first, the transfer is expected to finish in 200us. */
...
static void i2c_dw_xfer_init(struct dw_i2c_dev *dev)
{
	...
	dev->status |= STATUS_XFER_IN_PROGRESS;
...
i2c_dw_xfer_msg(struct dw_i2c_dev *dev)
{
	struct i2c_msg *msgs = dev->msgs;
	u32 intr_mask;
	int tx_limit, rx_limit, max_tx_limit;
	...
	/*
	 * Find limits one time before updating the FIFOs. It was found at stress test that
	 * DW_IC_TXFLR and DW_IC_RXFLR are not always get refreshed inside of the loop below.
	 * If empty space is not enough for all the messages, another interrupt will be triggered
	 * and the rest of the data will be put into FIFO.
	 *
	 * Limit the max_tx_limit to half the tx_fifo_depth.
	 * The DW_IC_INTR_TX_EMPTY is set if the entries inside tx fifo is less than half
	 * tx_fifo_depth. This can assert we never overflow the tx_fifo.
	 */
	max_tx_limit = (dev->tx_fifo_depth / 2);

	regmap_read(dev->map, DW_IC_TXFLR, &flr);
	tx_limit = dev->tx_fifo_depth - flr;
	tx_limit = min(tx_limit, max_tx_limit);
...
static const unsigned int regs2read[] = { DW_IC_RAW_INTR_STAT, DW_IC_TXFLR,
						       DW_IC_RXFLR, DW_IC_STATUS,
						       DW_IC_ENABLE_STATUS, DW_IC_CON, DW_IC_TAR,
						       DW_IC_INTR_MASK, DW_IC_INTR_STAT };
static const char * const regs_names[] = { "RAWINT", "TXFLR", "RXFLR", "STAT", "EN_STAT",
...
static void dump_regs(struct dw_i2c_dev *dev)
{
	...
	dev_err(dev->dev, "regs:");
	for (int i = 0; i < DUMP_REGS_NUM; ++i) {
		regmap_read(dev->map, regs2read[i], &reg_val);
		pr_cont("%s:%#X ", regs_names[i], reg_val);
	}
	pr_cont("\n");
...
static bool i2c_dw_wait_for_xfer_completion(struct dw_i2c_dev *dev, bool busy_wait_mode)
{
	/* busy_wait_mode prevents context switches which improves performance */
	if (busy_wait_mode) {
		unsigned long start_time = ktime_get_ns();

		/* If busy waiting times out, fallback to wait_for_completion. */
		while (ktime_get_ns() - start_time < BUSY_WAIT_XFER_TIMEOUT_NS) {
			udelay(2);

			if (completion_done(&dev->cmd_complete))
				return false;
		}
	}

	return !wait_for_completion_timeout(&dev->cmd_complete, dev->adapter.timeout);
}
...
static void irq_state_show(struct dw_i2c_dev *dev)
{
	...
	ret_active = irq_get_irqchip_state(dev->irq, IRQCHIP_STATE_ACTIVE, &active);
	ret_masked = irq_get_irqchip_state(dev->irq, IRQCHIP_STATE_MASKED, &masked);

	dev_err(dev->dev, "irq %d, %s, %s, %s, desc->depth: %d\n",
		dev->irq,
...
i2c_dw_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	for (int i = 0; i < num; ++i)
		total_bytes += msgs[i].len + 1;

	busy_wait_mode = (dev->operation_mode == HYBRID_BUSY_WAIT_MODE)
				&& (total_bytes < dev->hybrid_threshold);
	...
	if (i2c_dw_wait_for_xfer_completion(dev, busy_wait_mode)) {
		unsigned int tar_reg, imr_reg, rawint_reg, irstat_reg;
		...
		regmap_read(dev->map, DW_IC_TAR, &tar_reg);
		regmap_read(dev->map, DW_IC_INTR_MASK, &imr_reg);
		regmap_read(dev->map, DW_IC_RAW_INTR_STAT, &rawint_reg);
		regmap_read(dev->map, DW_IC_INTR_STAT, &irstat_reg);

		if (tar_reg == 0)
			panic("i2c controller timed out. Registers not accessible.\n");

		if (imr_reg == DW_IC_INTR_MASTER_MASK &&
		    rawint_reg == DW_IC_INTR_TX_EMPTY &&
		    !dev->irq_counter) {
			dev_err(dev->dev, "i2c controller timed out. IRQ fired but handler not called.\n");
			dev_err(dev->dev, "RAWINT 0x%08x, TAR 0x%08x, IR_MASK 0x%08x, IR_STAT 0x%08x, irq counter %d\n",
				rawint_reg, tar_reg, imr_reg, irstat_reg, dev->irq_counter);
		}
...
static irqreturn_t i2c_dw_isr(int this_irq, void *dev_id)
{
	...
	if (dev->timeout) {
		dev_err(dev->dev, "IRQ after timeout\n");
		dev->timeout = false;
	}
...
tx_aborted:
	...
	if ((!(dev->status & STATUS_XFER_IN_PROGRESS) && dev->rx_outstanding == 0) ||
	     dev->msg_err)
...
int i2c_dw_probe_master(struct dw_i2c_dev *dev)
{
	...
	/* Mode of operation Busy-wait/Interrupt mode */
	if (of_property_read_bool(dev->dev->of_node, "hybrid-busy-wait-mode"))
		dev->operation_mode = HYBRID_BUSY_WAIT_MODE;
	else
		dev->operation_mode = INTERRUPT_MODE;

	dev->hybrid_threshold = HYBRID_BUSY_WAIT_BYTES_THLD;
	of_property_read_u32(dev->dev->of_node, "hybrid-threshold", &dev->hybrid_threshold);

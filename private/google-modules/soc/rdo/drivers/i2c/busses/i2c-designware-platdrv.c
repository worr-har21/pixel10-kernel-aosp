...
static int dw_i2c_plat_probe(struct platform_device *pdev)
{
	...
	if (!IS_ERR(dev->pinctrl)) {
		dev->cli_state = pinctrl_lookup_state(dev->pinctrl, "cli");
		if (IS_ERR(dev->cli_state))
			dev->cli_state = NULL;
	} else {
		dev->pinctrl = NULL;
	}
	...
	dev->ext_power_control = of_property_read_bool(pdev->dev.of_node, "external-power-control");
...
static const struct dev_pm_ops dw_i2c_dev_pm_ops = {
	...
	SYSTEM_SLEEP_PM_OPS(dw_i2c_plat_suspend, dw_i2c_plat_resume)

			    ...
			    struct cluster_config *cluster)
{
	 ...
	 */
	if (of_property_read_u32(cluster_node, "cpuidle_state_depth_threshold",
			&cluster->cpuidle_state_depth_threshold)) {
		dev_dbg(dev, "Config idle_aware mising. Defaulting to no idle awareness.\n");
		cluster->cpuidle_state_aware = false;
	}

	if (cluster->cpuidle_state_depth_threshold == PERF_CPU_IDLE_C1
	|| cluster->cpuidle_state_depth_threshold == PERF_CPU_IDLE_C2)

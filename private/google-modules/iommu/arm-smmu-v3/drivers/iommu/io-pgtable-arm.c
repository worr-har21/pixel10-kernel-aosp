...
struct io_pgtable_init_fns io_pgtable_arm_64_lpae_s1_init_fns_pixel = {
	.alloc		= arm_64_lpae_alloc_pgtable_s1,
	.free		= arm_lpae_free_pgtable,
	.configure	= arm_64_lpae_configure_s1,
};

struct io_pgtable_init_fns io_pgtable_arm_64_lpae_s2_init_fns_pixel = {
...
#ifdef CONFIG_IOMMU_IO_PGTABLE_LPAE_SELFTEST_PIXEL
...
static int __init arm_lpae_run_tests(struct io_pgtable_cfg *cfg)
{
	...
	for (i = 0; i < ARRAY_SIZE(fmts); ++i) {
		cfg_cookie = cfg;
		ops = alloc_io_pgtable_ops_pixel(fmts[i], cfg, cfg);
		if (!ops) {

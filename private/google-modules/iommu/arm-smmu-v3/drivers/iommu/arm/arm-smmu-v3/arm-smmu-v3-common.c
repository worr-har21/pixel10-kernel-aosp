...
void arm_smmu_print_evt_info(struct arm_smmu_device *smmu, u64 *evt, const char *master_name)
{
	...
	u32 ssid = ssid_valid ? FIELD_GET(EVTQ_0_SSID, evt[0]) : IOMMU_NO_PASID;
	const char * const evt_str[] = {
		[EVT_ID_BAD_STREAMID] = "StreamID",
		[EVT_ID_BAD_STE] = "STE",
		[EVT_ID_BAD_SUBSTREAMID] = "SubstreamID",
		[EVT_ID_BAD_CD] = "CD",
		[EVT_ID_TRANSLATION_FAULT] = "Translation",
		[EVT_ID_ADDR_SIZE_FAULT] = "Address",
		[EVT_ID_ACCESS_FAULT] = "Access flag",
		[EVT_ID_PERMISSION_FAULT] = "Permission",
	};
	...
	case EVT_ID_BAD_CD:
		dev_err(smmu->dev, "Bad %s: sid %#x.%#x client %s\n",
			evt_str[id], sid, ssid, master_name);
		break;
	case EVT_ID_TRANSLATION_FAULT:
	case EVT_ID_ADDR_SIZE_FAULT:
	case EVT_ID_ACCESS_FAULT:
	case EVT_ID_PERMISSION_FAULT:
		dev_err(smmu->dev, "%s fault: sid %#x.%#x client %s iova %#llx %s %s %s %s class %s IPA %#llx\n",
			evt_str[id], sid, ssid, master_name,
			...
			FIELD_GET(EVTQ_1_S2, evt[1]) ? "S2" : "S1",
			class_str[FIELD_GET(EVTQ_1_CLASS, evt[1])],
			FIELD_GET(EVTQ_3_IPA, evt[3]));
		break;
	}
}
...
{
	u32 sysdisc9, scr, status;
	unsigned int num_tbu, gnt_xlate_slots;
	unsigned long long connected_tbus = 0;
	int i;

	scr = readl_relaxed(smmu->base + ARM_MMU700_TCU_SCR);
	if (!(scr & TCU_SCR_NS_UARCH)) {
		dev_err(smmu->dev, "No access to microarchitectural registers\n");
		return;
	}

	sysdisc9 = readl_relaxed(smmu->base + ARM_MMU700_TCU_SYSDISC9);
	num_tbu = FIELD_GET(TCU_SYSDISC9_TCUCFG_NUM_TBU, sysdisc9);

	status = readl_relaxed(smmu->base + ARM_MMU700_TCU_STATUS);
	gnt_xlate_slots = FIELD_GET(TCU_STATUS_GNT_XLATE_SLOTS, status);

	num_tbu = min_t(typeof(num_tbu), num_tbu, sizeof(connected_tbus) * 8);
	for (i = 0; i < num_tbu; i++) {
		u32 node_status = readl_relaxed(smmu->base + ARM_MMU700_TCU_NODE_STATUS(i));

		if (node_status & TCU_NODE_STATUS_CONNECTED)
			connected_tbus |= BIT(i);
	}

	dev_info(smmu->dev, "gnt_xlate_slots: %u connected TBUs: %04llx\n",
		 gnt_xlate_slots, connected_tbus);
}
				   ...
				   dma_addr_t l2ptr_dma)
{
	...
	val |= FIELD_PREP(STRTAB_L1_DESC_SPAN, smmu->strtab_cfg.split + 1);
...
static int arm_smmu_init_strtab_2lvl(struct arm_smmu_device *smmu)
{
	...
	/* l1size needs to be page aligned for pKVM SMMU */
...
static int arm_smmu_init_strtab_linear(struct arm_smmu_device *smmu)
{
	...
	/* size needs to be page aligned for pKVM SMMU */

...
struct syna_cdev_data {
...
#ifdef PID_TASK
	/* This variable is used to store the PID communicating with */
	pid_t proc_pid;
#endif
	...
	/* This variable allows caller to ask extra bytes to read appending
	 ...
	 * If set to '0', it represents no limitation and the depth of FIFO which is
	 * equal to FIFO_QUEUE_MAX_FRAMES.
	 */
...
static void syna_cdev_clean_fifo(struct syna_tcm *tcm)
{
	...
	while (!list_empty(&tcm->frame_fifo_queue)) {
		...
		syna_pal_mem_free(pfifo_data->fifo_data);
		syna_pal_mem_free(pfifo_data);
...
/**
 * @brief   Push one data packet to the kernel fifo.
 *
 * @param
 *    [ in] tcm:      the driver handle
	...
	unsigned char *buf_ptr, unsigned int length)
{
	...
	if (tcm->fifo_remaining_frame >= FIFO_QUEUE_MAX_FRAMES) {
		...
		syna_pal_mem_free(pfifo_data_temp->fifo_data);
		syna_pal_mem_free(pfifo_data_temp);
	...
	pfifo_data = syna_pal_mem_alloc(1, sizeof(*pfifo_data));
	if (!(pfifo_data)) {
	...
	pfifo_data->fifo_data = syna_pal_mem_alloc(length, sizeof(u8));
...
/**
 * @brief   Queue the specified data packet to the kernel fifo.
...
/**
 * @brief   Common helper to handle the reports.
...
/**
 * @brief   Perform the hardware reset.
 *          Optional arguments:
...
/**
 * @brief   To store the userspace application information.
...
/**
 * @brief   Check the queuing status of kernel fifo through IOCTL interface.
 *
 * @param
 *    [ in] tcm:       the driver handle
...
/**
 * @brief   Wrapper to clean the kernel fifo.
 *
 * @param
 *    [ in] tcm: the driver handle
 *
...
/**
 * @brief   Read out the data from the kernel fifo and copy to
 *          the userspace through IOCTL interface.
 *
 * @param
 *    [ in] tcm:           the driver handle
 *    [out] ubuf_ptr:      buffer of memory space from userspace;
...
/**
 * @brief   Assign the types of message for queuing through IOCTL interface.
	...
	for (idx = 0; idx < MAX_REPORT_TYPES; idx++) {
		if (data[idx] == 1) {
			...
			retval = syna_tcm_set_data_duplicator(tcm->tcm_dev,
					(unsigned char) idx,
					syna_cdev_process_reports, (void *)tcm);
			if (retval < 0) {
...
/**
 * @brief   Process the message through IOCTL interface.
 *          Caller can config the way to process through io_polling_interval.
 *
 * @param
 *    [ in]     tcm:       the driver handle
	...
	unsigned int *msg_size)
{
	...
	if (g_cdev_data.io_polling_interval == CMD_RESPONSE_IN_ATTN)
		resp_handling = CMD_RESPONSE_IN_ATTN;
	else
		resp_handling = g_cdev_data.io_polling_interval;
...
/**
 * @brief   Enable or disable the kernel irq through IOCTL interface.
 *
 *          Expect to get 4 bytes unsigned int parameter from userspace:
 *            0:         disable the irq.
 *            1:         enable the irq and set io_polling_interval as RESP_IN_ATTN
	...
	unsigned int data_size)
{
	...
	case 0:
		...
		g_cdev_data.io_polling_interval = tcm->tcm_dev->msg_data.command_polling_time;
	...
	case 1:
		...
		g_cdev_data.io_polling_interval = CMD_RESPONSE_IN_ATTN;
...
/**
 * @brief   Save PID through IOCTL interface.
 *
 * @param
 *    [ in] tcm:       the driver handle
 *    [ in] ubuf_ptr:  buffer of memory space from userspace
	...
	unsigned int data_size)
{
	...
	g_cdev_data.proc_pid = syna_pal_le4_to_uint(&data[0]);
	LOGD("PID: %d\n", (unsigned int)g_cdev_data.proc_pid);
	if (g_cdev_data.proc_pid) {
		proc_task = pid_task(find_vpid(g_cdev_data.proc_pid), PIDTYPE_PID);
		if (!proc_task) {
...
/**
 * @brief   Read out the data from device directly without routing to the
...
/**
 * @brief   Write the data to device directly without routing to the
 *          command handling module.
 *
 * @param
 *    [ in] tcm:      the driver handle
...
/**
 * @brief   Query the current configuration of driver.
 *
 * @param
 *    [ in] tcm:      the driver handle
 *    [out] ubuf_ptr: buffer of memory space from userspace
	...
	unsigned int size)
{
	...
	struct tcm_buffer *caller;
	struct tcm_dev *tcm_dev = tcm->tcm_dev;
	struct tcm_identification_info *id = &tcm_dev->id_info;
	...
	param->bus.max_wr_size = (unsigned short)tcm_dev->max_wr_size;
	param->bus.max_rd_size = (unsigned short)tcm_dev->max_rd_size;
	...
	if (id->version > 0)
		param->connection.touchcomm_version = (unsigned char)id->version;
...
/**
 * @brief   Set up the configuration of driver.
 *
 * @param
 *    [ in] tcm:      the driver handle
 *    [ in] ubuf_ptr: buffer of memory space from userspace
	...
	unsigned int in_size)
{
	...
	if (tcm->is_connected) {
		...
		if (param->bus.max_rd_size > 0) {
			if (tcm_dev->max_rd_size != param->bus.max_rd_size)
				chunks_update = true;
		}
		if (param->bus.max_wr_size > 0) {
			if (tcm_dev->max_wr_size != param->bus.max_wr_size)
				chunks_update = true;
		}
		if (chunks_update)
			tcm_dev->set_max_rw_size(tcm_dev,
				param->bus.max_wr_size, param->bus.max_rd_size);
...
/**
 * @brief   Dispatch the IOCTLs and execute the associated operations.
 *
 * @param
 *    [ in] tcm:       the driver handle
	...
	unsigned int ubuf_size, unsigned int *data_size)
{
	...
	switch (code) {
	case STD_SET_PID_ID:
		retval = syna_cdev_ioctl_store_pid(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	case STD_ENABLE_IRQ_ID:
		retval = syna_cdev_ioctl_enable_irq(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	case STD_RAW_WRITE_ID:
		retval = syna_cdev_ioctl_raw_write(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	case STD_RAW_READ_ID:
		retval = syna_cdev_ioctl_raw_read(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	case STD_GET_FRAME_ID:
		retval = syna_cdev_ioctl_get_frame(tcm, ubuf_ptr,
			ubuf_size, data_size);
		break;
	case STD_SEND_MESSAGE_ID:
		retval = syna_cdev_ioctl_send_message(tcm, ubuf_ptr,
			ubuf_size, data_size);
		break;
	case STD_SET_REPORTS_ID:
		retval = syna_cdev_ioctl_set_queued_types(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	case STD_CHECK_FRAMES_ID:
		retval = syna_cdev_ioctl_check_frame(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	...
	case STD_APPLICATION_INFO_ID:
		retval = syna_cdev_ioctl_application_info(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	case STD_DO_HW_RESET_ID:
		retval = syna_cdev_ioctl_do_hw_reset(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	case STD_DRIVER_CONFIG_ID:
		retval = syna_cdev_ioctl_set_config(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	case STD_DRIVER_GET_CONFIG_ID:
		retval = syna_cdev_ioctl_get_config_params(tcm, ubuf_ptr,
			ubuf_size, *data_size);
		break;
	default:
...
/**
 * @brief   Handle the old IOCTLs that is used in the legacy driver.
 *
 * @param
 *    [ in] tcm:      the driver handle
 *    [ in] code:     code for the target operation
...
/**
 * @brief   Entry of the IOCTL operations.
 *
 * @param
...
#ifdef USE_COMPAT_IOCTL
/**
 * @brief   Entry of the compatible IOCTL operations.
 *
 * @param
 *    [ in] filp: represents the file descriptor
 *    [ in] cmd: command code sent from userspace
...
/**
 * @brief   Used to change the current position in a file.
 *
 * @param
 *    [ in] filp:   represents the file descriptor
...
/**
 * @brief   Used to read data through the device file.
...
/**
 * @brief   Used to write data to device through the device file.
...
/**
 * @brief   Invoked when the device file is being open, which should be
...
/**
 * @brief   Invoked when the device file is being released
 *
 * @param
 *    [ in] inp:  represents a file in rootfs
 *    [ in] filp: represents the file descriptor
...
/**
 * @brief   Provide the declaration of devtmpfs.
 *
 * @param
...
/**
 * @brief   Create a device node and register the sysfs attribute.
 *
 * @param
 *    [ in] tcm: the driver handle
 ...
 */
int syna_cdev_create(struct syna_tcm *tcm, struct platform_device *pdev)
{
	int retval = 0;
...
/**
 * @brief   Remove the cdev device node allocated previously.
 *
 * @param
 *    [ in] tcm: the driver handle
 *
